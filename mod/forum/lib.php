...
define('FORUM_MAILED_SKIPPED', 1);
define('FORUM_MAILED_SUCCESS', 2);
    ...
    if ($posts) {
        foreach ($posts as $post) {
            ...
            if ($errorcount[$post->id]) {
                $DB->set_field("forum_posts", "mailed", FORUM_MAILED_SUCCESS, array("id" => "$post->id"));
            }
        }
    }
...
function forum_mark_old_posts_as_mailed($endtime, $now=null) {
    ...
    if (empty($CFG->forum_enabletimedposts)) {
        return $DB->execute("UPDATE {forum_posts}
                               SET mailed = ".FORUM_MAILED_SKIPPED."
